...
| **IP Elástico**                           | É uma configuração de IP que permite reiniciar a máquina e não perder o IP público. Permite também migrar esse IP para outra máquina(OBS: ao terminar uma instância deve-se apagar o IP público caso não vá reutilizar para não gerar custos.) Oferece instabilidade, acesso remoto consistente. Ajuda em Load Balancers, para ajudar a distribuir o tráfego entre várias instâncias. | ![Ip Elástico](./images/21.gif) |
